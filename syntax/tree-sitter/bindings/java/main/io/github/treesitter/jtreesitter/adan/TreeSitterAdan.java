package io.github.treesitter.jtreesitter.adan;

import java.lang.foreign.*;

public final class TreeSitterAdan {
    ...
    private static final TreeSitterAdan INSTANCE = new TreeSitterAdan();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterAdan() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_adan");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-adan");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-adan");
